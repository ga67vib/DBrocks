					...
					<table style="width: 100%">
						...
						<tr>
							...
							<%
								if (request.getSession().getAttribute("visitor") != null) { //only display Select column for visitors
							%>
							<td><input type="checkbox" name="selected19<%=i%>"
								value="<%=bean19.getChild(i).getPerformanceID()%>" /></td>
							<%
								}
							%>
						</tr>
					...
					<table style="width: 100%">
						...
						<tr>
							...
							<%
								if (request.getSession().getAttribute("visitor") != null) { //only display Select column for visitors
							%>
							<td><input type="checkbox" name="selected20<%=i%>"
								value="<%=bean20.getChild(i).getPerformanceID()%>" /></td>
							<%
								}
							%>
						</tr>
